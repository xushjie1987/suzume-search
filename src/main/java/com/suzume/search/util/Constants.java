/**
 * Project Name:suzume-search
 * File Name:Constants.java
 * Package Name:com.suzume.search.util
 * Date:2016年4月3日下午1:27:53
 * Copyright (c) 2016, All Rights Reserved.
 *
 */

package com.suzume.search.util;

/**
 * ClassName:Constants <br/>
 * Function: <br/>
 * Date: 2016年4月3日 下午1:27:53 <br/>
 * 
 * @author shengjie
 * @version
 * @since JDK 1.7
 * @see
 */
public class Constants {
    
    public static final String DATE_PATTERN = "yyyy-MM-dd";
    
    public static final String HUMAN_DATE   = "yyyy年MM月dd日 HH时mm分ss秒";
    
    public static final String OS_CHARSET   = "sun.jnu.encoding";
    
    public static final String FILE_CHARSET = "file.encoding";
    
}
